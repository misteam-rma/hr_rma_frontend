...
const HodMaster = () => {
  ...
  const handleDelete = async (hod) => {
    if (!window.confirm(`Delete HOD "${hod.hodName}"?`)) return;

    try {
      await deleteHodApi(hod.id);
      ...
      <div className="overflow-hidden border border-gray-200 rounded-2xl bg-white shadow-sm flex flex-col">
            ...
            <table className="min-w-full">
              <thead className="bg-gray-50/80 sticky top-0 z-10 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-[11px] font-bold text-gray-400 uppercase tracking-widest whitespace-nowrap">HOD Name</th>
                ...
                {tableLoading ? (
                  ...
                  <tr>
                    <td colSpan="3" className="px-6 py-24 text-center">
                      <p className="text-gray-500 text-lg">No HOD data found.</p>
                    </td>
                  </tr>
                ) : (
                  paginatedData.map((item, index) => (
                    <tr key={item.id} className="hover:bg-indigo-50/40 transition-colors">
                      ...
                      <td className="px-6 py-5 text-sm font-bold text-gray-900 whitespace-nowrap">{item.hodName}</td>
        ...
        <div className="md:hidden flex flex-col h-[calc(100vh-220px)]">
          <div className="flex-1 p-2 space-y-3 overflow-y-auto scrollbar-hide">
            {tableLoading ? (
              <LoadingSpinner message="Retrieving HODs..." minHeight="250px" />
            ) : paginatedData.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-32">
                <div className="p-4 bg-gray-100 rounded-full mb-3">
                  <Search size={24} className="text-gray-400" />
                </div>
                <p className="text-gray-500 font-bold text-sm tracking-tight text-center px-6 uppercase">No matching HODs found</p>
              </div>
            ) : (
