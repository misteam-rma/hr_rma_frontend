...
const HodMaster = () => {
  ...
  const loadHODs = async () => {
    ...
    if (result.success) {
      ...
      toast.error('Failed to load HODs: ' + result.error);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hodName.trim()) {
      toast.error('Please enter a HOD name');
      return;
    }
    ...
    try {
      setSubmitting(true);
      if (editingId) {
        await updateHodApi(editingId, hodName.trim());
        toast.success('HOD updated successfully!');
      } else {
        await createHodApi(hodName.trim());
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteHodApi(deleteTarget.id);
  ...
  const filteredData = hods.filter((item) =>
    searchTerm ? item.hodName.toLowerCase().includes(searchTerm.toLowerCase()) : true
  );
  ...
  const headers = ['Serial No', 'HOD Name', { label: 'Actions', className: 'w-28' }];

  const renderRow = (item, index) => (
    <tr key={item.id} className="hover:bg-blue-50/40 transition-colors">
      <td className="px-4 py-3 whitespace-nowrap text-xs text-gray-400 text-center">{pageStart + index + 1}</td>
      <td className="px-4 py-3 text-xs font-bold text-gray-900 whitespace-nowrap">{item.hodName}</td>
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 flex items-center justify-between gap-4 group hover:border-blue-200 transition-all duration-300">
      <div className="text-sm font-black text-gray-800 leading-tight truncate uppercase tracking-tight">{item.hodName}</div>
  ...
  return (
    <div className="space-y-3 md:pb-4 mb-4">
      <div className="hidden md:flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">HOD Master</h1>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 bg-white border border-gray-200 rounded-2xl shadow-sm px-3 py-2.5">
        ...
        <div className="flex items-center gap-2 justify-end sm:ml-auto">
          <button onClick={loadHODs} className="inline-flex items-center justify-center p-2 border border-gray-300 rounded-lg text-gray-600 bg-white hover:bg-gray-50 transition-all duration-200 shrink-0" title="Refresh" disabled={tableLoading}>
            <RefreshCcw size={16} className={tableLoading ? 'animate-spin' : ''} />
          </button>
          <button onClick={openCreateModal} className="inline-flex items-center justify-center px-3.5 py-2 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-all duration-200 shrink-0" title="Add HOD">
      ...
      <ModalForm
        ...
        title={editingId ? 'Edit HOD' : 'Add New HOD'}
      ...
      <ConfirmationModal
        ...
        type="danger"
        title="Delete HOD"
        message={`Are you sure you want to delete "${deleteTarget?.hodName}"? This cannot be undone.`}
        confirmText="Delete"
      />

      <div className="border border-gray-200 rounded-2xl bg-white shadow-sm overflow-hidden h-[calc(100vh-260px)] min-h-[420px]">
        {tableLoading ? (
          <LoadingSpinner message="Loading HODs..." minHeight="420px" />
        ) : (
          <DataTable
            headers={headers}
            data={paginatedData}
