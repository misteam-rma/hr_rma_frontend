...
import {
  ...
  updateDepartmentApi,
  deleteDepartmentApi,
} from '../../utils/departmentMasterApi';
...
const DepartmentMaster = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      if (editingId) {
        await updateDepartmentApi(editingId, departmentName.trim());
        toast.success('Department updated successfully!');
      } else {
        await createDepartmentApi(departmentName.trim());
  ...
  const filteredData = departments.filter((item) =>
    searchTerm ? item.departmentName.toLowerCase().includes(searchTerm.toLowerCase()) : true
  );
  ...
  const headers = ['Serial No', 'Department Name', { label: 'Actions', className: 'w-28' }];

  const renderRow = (item, index) => (
    <tr key={item.id} className="hover:bg-blue-50/40 transition-colors">
      <td className="px-4 py-3 whitespace-nowrap text-xs text-gray-400 text-center">{pageStart + index + 1}</td>
      <td className="px-4 py-3 text-xs font-bold text-gray-900 whitespace-nowrap">{item.departmentName}</td>
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 flex items-center justify-between gap-4 group hover:border-blue-200 transition-all duration-300">
      <div className="text-sm font-black text-gray-800 leading-tight truncate uppercase tracking-tight">{item.departmentName}</div>
    ...
    <div className="space-y-3 md:pb-4 mb-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 bg-white border border-gray-200 rounded-2xl shadow-sm px-3 py-2.5">
        ...
        <div className="flex items-center gap-2 justify-end sm:ml-auto">
          <button onClick={loadDepartments} className="inline-flex items-center justify-center p-2 border border-gray-300 rounded-lg text-gray-600 bg-white hover:bg-gray-50 transition-all duration-200 shrink-0" title="Refresh" disabled={tableLoading}>
            <RefreshCcw size={16} className={tableLoading ? 'animate-spin' : ''} />
          </button>
          <button onClick={openCreateModal} className="inline-flex items-center justify-center px-3.5 py-2 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-all duration-200 shrink-0" title="Add Department">
      ...
      <div className="border border-gray-200 rounded-2xl bg-white shadow-sm overflow-hidden h-[calc(100vh-260px)] min-h-[420px]">
        {tableLoading ? (
          <LoadingSpinner message="Loading departments..." minHeight="420px" />
        ) : (
          <DataTable
            headers={headers}
            data={paginatedData}
