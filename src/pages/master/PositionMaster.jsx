...
const PositionMaster = () => {
  ...
  const headers = ['Serial No', 'Position Name', { label: 'Actions', className: 'w-28' }];

  const renderRow = (item, index) => (
    <tr key={item.id} className="hover:bg-blue-50/40 transition-colors">
      <td className="px-4 py-3 whitespace-nowrap text-xs text-gray-400 text-center">{pageStart + index + 1}</td>
      <td className="px-4 py-3 text-xs font-bold text-gray-900 whitespace-nowrap">{item.positionName}</td>
    ...
    <div className="space-y-3 md:pb-4 mb-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 bg-white border border-gray-200 rounded-2xl shadow-sm px-3 py-2.5">
        ...
        <div className="flex items-center gap-2 justify-end sm:ml-auto">
          <button onClick={loadPositions} className="inline-flex items-center justify-center p-2 border border-gray-300 rounded-lg text-gray-600 bg-white hover:bg-gray-50 transition-all duration-200 shrink-0" title="Refresh" disabled={tableLoading}>
            <RefreshCcw size={16} className={tableLoading ? 'animate-spin' : ''} />
          </button>
          <button onClick={openCreateModal} className="inline-flex items-center justify-center px-3.5 py-2 border border-transparent rounded-lg shadow-sm text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 transition-all duration-200 shrink-0" title="Add Position">
      ...
      >
        <div>
          <label className={labelClass}>Position Name *</label>
          <input
            type="text"
            value={positionName}
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to delete "${deleteTarget?.positionName}"? This cannot be undone.`}
        confirmText="Delete"
      />

      <div className="border border-gray-200 rounded-2xl bg-white shadow-sm overflow-hidden h-[calc(100vh-260px)] min-h-[420px]">
        {tableLoading ? (
          <LoadingSpinner message="Loading positions..." minHeight="420px" />
        ) : (
          <DataTable
            headers={headers}
            data={paginatedData}
